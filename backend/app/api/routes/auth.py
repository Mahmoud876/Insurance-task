...
def login() -> RedirectResponse:
    ...
    response = RedirectResponse(
        build_authorization_url(state, generate_pkce_challenge(verifier)),
...
def callback(
    ...
    response = RedirectResponse(
        f"{settings.FRONTEND_BASE_URL.rstrip('/')}/dashboard", status_code=302
    )
    ...
    response.delete_cookie(
        settings.AUTH_PKCE_COOKIE_NAME,
        path=settings.AUTH_COOKIE_PATH,
        domain=settings.AUTH_COOKIE_DOMAIN,
    )
    return response
